...
// ---------------------------------------------------------------------------
// 🚀 [기능 1] 학생 문장 평가 (evaluateAnswer) - 변경 없음
// ---------------------------------------------------------------------------
export async function evaluateAnswer(
  ...
  const prompt = `
    You are a warm, encouraging, and highly precise native English tutor.
    Your student, ${studentName}, is learning to speak natural English.
    ...
    3. 🛑 LOGIC CHECK (논리 검증): 문법(수 일치, 시제 등)을 설명할 때는 원문의 주어/동사와 사용자의 주어/동사를 정확히 구분하고, 앞뒤 설명 논리가 모순되지 않는지 반드시 스스로 한 번 더 체크하세요.
    4. 🛑 FACT-BASED FEEDBACK (팩트 기반 피드백): 억측이나 과장 없이, 두 문장의 차이점(단어 선택, 스펠링, 문법)을 있는 그대로 객관적이고 명확하게 지적하는 데 집중하세요.
    5. Compare the student's answer with the Target Sentence strictly based on the rules above.
    6. If the meaning, nuance, and grammar are highly natural and effectively convey the intended context, mark 'isPass: true'.
    7. If there are awkward phrasing, grammatical errors, or contextual mismatches, mark 'isPass: false'.
    8. The feedback must be written in friendly Korean, addressing the student as "${studentName}".
    9. ⚠️ REQUIRED FEEDBACK FORMAT: You MUST structure the 'feedback' string exactly with these TWO headings.
    Make it detailed but easy to read. Do NOT include an overall assessment (평가 결과) heading here.
    **💡 문맥적 분석**
       (Write how well the answer fits the situation/context here)

       **📝 상세 평가**
       (Write specific grammar and vocabulary feedback here)
    10. Generate 2 similar, short, native-level English sentences that fit the same context.
...

    {
      ...
      "quickPraise": "string", // [중요] 이 항목은 전체 평가를 관통하는 '객관적이고 군더더기 없는 1줄 요약'입니다. 감정적인 인사말이나 억지 칭찬은 절대 금지합니다!
                               // - isPass가 true일 때: 객관적이고 명확한 1줄 칭찬 (예: '상황에 딱 맞는 완벽한 과거형 표현입니다.')
                               // - isPass가 false일 때: 객관적이고 부드러운 1줄 문제점 지적 (예: '시제는 좋았지만, 맥락에 맞지 않는 동사가 사용되었습니다.')
      "feedback": "string", // MUST include ONLY the TWO headings mentioned in rule #6 (문맥적 분석, 상세 평가), separated by line breaks (\\n\\n).
      "similarSentences": [
        { "en": "Similar sentence 1", "ko": "한국어 뜻" },
        { "en": "Similar sentence 2", "ko": "한국어 뜻" }
      ]
    }
  `;
...
// ---------------------------------------------------------------------------
// 🚀 [기능 2] 자동 힌트/상황 생성 (generateRecallTrigger) - 변경 없음
// ---------------------------------------------------------------------------
export async function generateRecallTrigger(
  ...
  const prompt = `
    ...
    [TASK]
    Translate the English 'Target Sentence' into natural, conversational Korean.
    If the 'Context' contains specific Korean hints or meanings, you MUST use them in your translation to help the user recall the exact English sentence.
    ...
    [RULES]
    1. 🎯 ESSENTIAL: The output must capture the EXACT meaning of the English sentence as a FULL, COMPLETE Korean sentence. Do not just output fragments.
    2. 🧩 CONTEXT INTEGRATION: If 'Context' is provided (e.g., "헐값", "모른 척하다"), you MUST weave that exact Korean word/phrase into the translation.
    ...
    5. 🚫 STRICT NO FILLER (절대 딴소리 금지): DO NOT add any prefixes like "번역문:", "힌트:", "문맥을 반영하면", or "주어진 문장은". Output absolutely NOTHING but the final translated Korean sentence itself.
...
// ---------------------------------------------------------------------------
// 🚀 [기능 3] 실시간 라이브 퀴즈 다중 생성 (generateLiveQuiz) - 배열 지원 업그레이드
// ---------------------------------------------------------------------------
export async function generateLiveQuiz(
  ...
  // 예외 처리: 선택된 문장이 없으면 빈 배열 반환
  if (!sentences || sentences.length === 0) return [];

  const model = genAI.getGenerativeModel({ model: MODEL_NAME });
  
  // 파라미터로 받은 객체 배열을 JSON 문자열로 변환하여 AI에게 전달
  const inputJSON = JSON.stringify(sentences, null, 2);

  const prompt = `
    당신은 영단어와 숙어를 가르치는 훌륭한 원어민 영어 선생님입니다.
    아래의 [입력 데이터 (배열)]을 바탕으로, 각 문장마다 학생들을 위한 4지선다형(객관식) 영어 빈칸 채우기 문제를 만들어주세요.

    [입력 데이터]
    ${inputJSON}

    [출력 규칙]
    1. 각 입력된 문장마다 '선생님의 힌트/키워드(userContext)'에 해당하는 핵심 표현(단어 또는 숙어) 부분을 빈칸 ( ____ )으로 뚫어서 문제를 출제하세요. (만약 키워드가 없다면 문장에서 가장 중요한 핵심 영단어나 숙어를 빈칸으로 만드세요.)
    2. 각 문제의 보기는 총 4개이며, 1개는 정답, 3개는 아주 그럴싸하고 헷갈리는 오답(유사한 형태의 숙어나 단어)으로 구성하세요. 보기의 배치 순서는 무작위로 섞어주세요.
    ...
    [
      {
        "questionText": "다음 한국어 뜻에 맞게 빈칸에 들어갈 알맞은 표현을 고르시오.",
        "koreanHint": "여기에 자연스러운 한국어 뜻 전체 작성",
        "blankSentence": "빈칸이 ( ____ ) 뚫린 영어 문장",
  ...
  try {
    const result = await model.generateContent({
      contents: [{ role: "user", parts: [{ text: prompt }] }],
      generationConfig: {
        temperature: 0.3, // 매력적인 오답 생성을 위해 약간의 창의성 허용
        maxOutputTokens: 2000, // 🚀 다중 생성에 대비하여 최대 응답 토큰 한도 대폭 상향
      },
      safetySettings,
    });
...
// ---------------------------------------------------------------------------
// 🚀 [기능 4] 맞춤형 오답 숙제 생성 (generateRemedialHomework)
// ---------------------------------------------------------------------------
...
export async function generateRemedialHomework(
  ...
  const prompt = `
    You are a creative English tutor.
    Based on the following JSON array of wrong sentences:
    ${inputJSON}

    For EACH sentence, create ONE similar, natural English sentence that uses the same core vocabulary/idiom or grammatical structure, but in a slightly different everyday context.

    [JSON OUTPUT FORMAT (MUST be an Array)]
    [
      {
        "originalSentenceId": "The 'id' from the input",
        "aiSentence": "The new similar English sentence",
...
// ---------------------------------------------------------------------------
// 🚀 [기능 5] 학생별 취약점 AI 진단 (analyzeStudentWeakness)
// ---------------------------------------------------------------------------
export async function analyzeStudentWeakness(
  ...
  const prompt = `
    You are an expert English tutor diagnosing a student's learning gaps.
    Based on the following JSON array of the student's wrong answers from a recent quiz:
    ${inputJSON}

    Analyze their common grammatical, vocabulary, or structural weaknesses.
    ...
    Example: "[${dateStr}] too~to 구문 오류, 수일치 실수 및 과거완료 시제 혼동"
